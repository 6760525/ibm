...
def model_monitor(country="all", dev=DEV, training=True):
    ...
    for n in samples:
        ...
        queries = [(d.year, d.month, d.day, country) for d in dates_new]
        y_pred = [model_predict(year=query[0], month=query[1], day=query[2], country=query[3],verbose=False, dev=True)["y_pred"][0].round(2) for query in queries]
